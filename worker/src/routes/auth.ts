...
app.post("/email/start", async (c) => {
  ...
  if (existing) {
    ...
    if (elapsed < RESEND_COOLDOWN_MS) {
      return c.json({ error: "rate_limited", retry_after: Math.ceil((RESEND_COOLDOWN_MS - elapsed) / 1000) }, 429);
    }
  }
  ...
  await c.env.DB
    ...
    .bind(email, codeHash, expiresAt, now.toISOString())
    .run();
...
app.post("/email/verify", async (c) => {
  ...
  if (!email || !code) return c.json({ error: "missing_fields" }, 400);
  ...
  if (row.attempts >= MAX_ATTEMPTS) return c.json({ error: "too_many_attempts" }, 400);
  ...
  if ((await hashCode(code)) !== row.code_hash) return c.json({ error: "invalid_code" }, 400);
